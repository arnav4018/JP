...
if (-not $step1Success) {
    Write-Host "Exiting due to Step 1 failure" -ForegroundColor Red
    exit 1
}
...
if (-not $step2Success) {
    Write-Host "Exiting due to Step 2 failure" -ForegroundColor Red
    exit 1
}
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        throw "Mock data loading failed with exit code $LASTEXITCODE"
    }
} catch {
...
if (-not $step3Success) {
    Write-Host "Exiting due to Step 3 failure" -ForegroundColor Red
    exit 1
}
